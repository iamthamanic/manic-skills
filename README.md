...
| `@issue-contract` | Kanonisches GitHub-Issue-Template (global) | **Situation:** Skills erstellen Issues mit eigenen Body-Shapes. **Was passiert:** Single source of truth für Issue-Struktur (Type→Intent→Goal→…→Runner); projekt-Override via `.qa/issue-template.md`, Werte via `.qa/project.yaml` → `issueContract`. **Beispiel:** `@feature-intake` erstellt Issue → Body folgt `@issue-contract` → `ecc-runner` seedet Acceptance daraus. | ✅ | ✅ | ✅ | ✅ |
| `@ecc-check` | Quality-Gate-Loop bis READY/BLOCKED | **Situation:** Feature fertig, willst wissen ob ready zum Committen. **Was passiert:** Phase A = `@test-gate`, B = verify-ticket, B2 = `@composition-gate`, dann `@review-ticket`, AgentShield, UI, Report. **Beispiel:** `@ecc-check` → `@test-gate` PASS → composition-gate CLEAR → `@review-ticket` ACCEPT → `READY`. | ✅ | ⚠️ | ✅ | ✅ |
...
| `@ecc-runner` | Autonomer GitHub-Issue-Queue-Runner | **Situation:** 5 offene Issues, automatisch abarbeiten. **Was passiert:** Bootstrapped Labels, baut Queue, arbeitet jedes Issue mit voller Pipeline (implement→verify→composition-gate→review→PR). **Beispiel:** `@ecc-runner` → Issue #42: implement→verify→composition-gate→review→PR #78 → Issue #43 → ... bis Queue leer. | ✅ | ⚠️ | ⚠️ | ⚠️ |
...
| `@composition-gate` | Hop-Ketten-Bedeutung (CLEAR/SKIPPED/FLAGGED) | **Situation:** Jede Datei sieht korrekt aus, der Pfad Producer→Consumer kann trotzdem falsch sein. **Was passiert:** Rekonstruiert 1–3 Events, drei Simulationen (N-Akteure, ungültiger Fallback, zwei Konsumenten/Crash). **FLAGGED muss gefixt werden.** Proof: `.qa/runs/composition-gate-<slug>.md`. **Beispiel:** 1 Announcement × 100 User → 100 Channel-Posts → `cardinality:` → Fix → CLEAR. | ✅ | ✅ | ✅ | ✅ |
...
| `@review-ticket` | Code-Quality-Review, ACCEPT/CHANGES_REQUESTED | **Situation:** Code fertig, Review vor PR. **Was passiert:** Architektur, Security, Diff-Scope. ACCEPT braucht composition-gate CLEAR/SKIPPED (gleiche HEAD-SHA). Ruft Subagents bei Bedarf. **Beispiel:** `@review-ticket` → "Diff 50 Zeilen, Acceptance 200" → `CHANGES_REQUESTED` → fix → `ACCEPT`. | ✅ | ⚠️ | ⚠️ | ⚠️ |
| `@commit-pr-safe` | Commit + Push + PR auf non-main | **Situation:** Quality Gate READY, commit+push+PR. **Was passiert:** Liest AGENTS.md, validiert non-main, Secret-Scan, AgentShield, README-Sync, composition-gate Proof (CLEAR/SKIPPED gleiche SHA) oder führt den Gate aus, PR. **Beispiel:** `@commit-pr-safe` → branch `feature/audio-export` → kein Secret → commit → push → PR #78. | ✅ | ✅ | ✅ | ✅ |
...
| `@pr-merge-safe` | PR reviewen + mergen wenn green | **Situation:** PR offen, CI läuft, reviewen+mergen. **Was passiert:** verify-ticket, composition-gate (oder same-SHA Proof), review-ticket, ecc-check, babysit. Merged bei User-OK. **Beispiel:** `@pr-merge-safe` → CI green → composition CLEAR → review ACCEPT → "Merge? y" → merged. | ✅ | ⚠️ | ⚠️ | ⚠️ |
