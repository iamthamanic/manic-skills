...
description: >-
  Elite mobile app image-generation for premium, app-native screen concepts and
  flows (onboarding, auth, home, detail, checkout, settings, chat). Use when
  working on mobile apps, iOS/Android/Capacitor/React Native/Flutter UI concepts,
  phone mockups, multi-screen app flows, or when the user asks for mobile screen
  images, app concepts, or redesign mockups. Prioritizes clean hierarchy, readable
  text, multi-screen consistency, controlled palettes, non-generic art direction,
  and content-first phone framing. Generates images only — does not write code.
  For web landing/portfolio aesthetics use design-taste-frontend; for UI code
  aesthetics use frontend-design.
---
